...
from app.utils import estimate_tdee, generate_recommendation, search_usda_food, calculate_progress_stats
...
def user_dashboard():
    ...
    if not rec or (datetime.now(timezone.utc) - rec.timestamp.replace(tzinfo=timezone.utc)).total_seconds() > 3600:
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Log initial weight
        log = WeightLog(user_id=user.id, weight=user.weight, date=datetime.now(timezone.utc))
        db.session.add(log)
        db.session.commit()
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        if not WeightLog.query.filter_by(user_id=current_user.id).first():
            log = WeightLog(user_id=current_user.id, weight=current_user.weight, date=datetime.now(timezone.utc))
            db.session.add(log)
...
def create_recommendation():
    ...
    rec = Recommendation(user_id=current_user.id, meal_rec=meal, workout_rec=workout, trend_note=trend_note)
    db.session.add(rec)
    db.session.commit()

    flash("New recommendation generated!", "success")
    return redirect(url_for('user_dashboard', ts=datetime.now(timezone.utc).timestamp()))
...
def recommendation_history():
    ...
    return render_template('recommendation_history.html', user=current_user, recommendations=recommendations)
...
@login_required
def previous_meals():
    meals = Meal.query.filter_by(user_id=current_user.id).order_by(Meal.id.desc()).limit(10).all()
...
def progress():
    ...
    weight_logs = WeightLog.query.filter_by(user_id=current_user.id).order_by(WeightLog.date.asc()).all()
    ...
    avg_macros, total_protein, total_carbs, total_fats = calculate_progress_stats(current_user)

    return render_template("progress.html", user=current_user, workouts=workouts, meals=meals, tdee=tdee, avg_macros=avg_macros, total_protein=total_protein, total_carbs=total_carbs, total_fats=total_fats, weight_logs=weight_logs, weight_labels=weight_labels, weight_values=weight_values)
...
def log_weight():
    ...
    if form.validate_on_submit():
        ...
        # Save to weight log
        log = WeightLog(user_id=current_user.id, weight=new_weight, date=datetime.now(timezone.utc))
        db.session.add(log)
...
def previous_workouts():
    ...
    return render_template('previous_workouts.html', workouts=workouts, user=current_user)
